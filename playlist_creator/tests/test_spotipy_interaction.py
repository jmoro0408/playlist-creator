...
def test_get_users_liked_tracks_fewer_than_limit(client_fixture):
    limit = 25
    assert len(client_fixture.get_user_liked_tracks_limited(limit=limit)) == limit


def test_get_users_liked_tracks_greater_than_limit(client_fixture):
    errors = []
    # testing with no track leftover, i.e divmod(len, 50) = 2,0
    limit_no_leftover = 100
    results = client_fixture.get_user_liked_tracks_limited(limit=limit_no_leftover)
    if len(results) != limit_no_leftover:
        _msg = f"{len(results)} tracks returned, expected {limit_no_leftover}"
        errors.append(_msg)
    # testing with track leftover, i.e divmod(len, 50) = 2,1
    limit_with_leftover = 101
    results = client_fixture.get_user_liked_tracks_limited(limit=limit_with_leftover)
    if len(results) != limit_with_leftover:
        _msg = f"{len(results)} tracks returned, expected {limit_with_leftover}"
        errors.append(_msg)

    assert len(errors) == 0


def test_get_users_playlists_info_type(client_fixture):
    assert isinstance(client_fixture.get_users_playlists_info(), dict)


def test_get_users_playlists_info_not_empty(client_fixture):
    assert len(client_fixture.get_users_playlists_info()) > 0


def test_get_user_playlist_track_info(client_fixture):
    errors = []
    first_playlist_name = list(client_fixture.get_users_playlists_info())[0]
    test_playlist_id = client_fixture.get_users_playlists_info()[first_playlist_name]
...
def test_get_track_features_multiple_tracks(client_fixture):
    errors = []
    first_playlist_name = list(client_fixture.get_users_playlists_info())[0]
    first_playlist_id = client_fixture.get_users_playlists_info()[first_playlist_name]
