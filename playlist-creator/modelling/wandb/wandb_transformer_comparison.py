...
classifiers = [LogisticRegression(max_iter = 1200),
               MLPClassifier(max_iter = 1000),
               RandomForestClassifier()]

total_config = len(scalers) * len(samplers) *len(featurisers)*len(classifiers)

config_permuation_builer = {'scaler':scalers,
          'sampler':samplers,
...
wandb.init(project = 'spotify-recommender',
           tags = ['transform'],
           name = "transformer comparisons")
...
config_df = pd.DataFrame(_values_list,
                                   columns =
                                   ["scaler",
                                    "sampler",
                                    "featuriser",
