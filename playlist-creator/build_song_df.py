...
def explode_results_list(result_list:list[tuple]) -> tuple:
...
def build_liked_song_df(sp: SpotipyClient, limit: int = 10) -> pd.DataFrame:
    ...
    track_names,artist_names,track_ids =  explode_results_list(parsed_liked_tracks_json)
...
def build_playlists_df(sp: SpotipyClient) -> pd.DataFrame:
    ...
    for playlist_name, playlist_id in user_playlists.items():
        playlist_tracks = sp.get_user_playlist_track_info(playlist_id)
